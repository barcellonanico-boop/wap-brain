...
Nico responds via dictation tool (MacWhisper or equivalent) producing a text file. Format:

- **Input format:** dictated text (`.md` or `.txt`), NOT audio
- **Language:** Italian, English, or mixed — Nico's choice
- **File location:** `projects/POST_[article-slug]/05b_Nico_Raw.md`
- **Structure:** unstructured monologue. Nico responds to questions but also adds tangents, ironies, family stories, opinions. Not organized by H2.

The Brain Dump is RAW MATERIAL, not structured content. It captures Nico's voice, knowledge, on-the-ground reality, ironies, and personal references. The downstream phases (Struttura, Bozza Informativa) interpret and integrate this material — they don't copy verbatim.

#### Step 7 — Light cleanup and organize

The Brain Dump arrives as raw dictated text. Architect agent processes it:

**Light cleanup pass (required):**
- Fix obvious typos and dictation tool storpiature (especially Italian words, dialect terms, proper names)
- Examples: "polit-eama" → "Politeama", "nonna nun-zia" → "Nonna Nunzia"
- DO NOT change meaning, voice, or ironies — only fix mechanical errors
- DO NOT remove filler/repetitions/tangents — those are voice signal

**Organization:**
- Match each Question Set question with the relevant Brain Dump segment
- Some questions may not have direct response — Nico may have answered tangentially or skipped
- Mark explicitly: MATCHED (clear response), TANGENTIAL (response indirectly relevant), SKIPPED (no response)
- Tangents and off-topic material → "Bonus / Off-Topic" section, preserved verbatim
- WAP_12 affiliate items mentioned → dedicated update list

**Critical:** Brain Dump is one of FOUR sources for the article. Other three: existing article (Phase 3 audit), AI knowledge, Search Intent (Phase 2). The Architect organizes Brain Dump material so downstream Copywriter can integrate it with the other sources, not treat it as the only source.
...
Phases 6-11 are execution: Struttura, Bozza Informativa, Voice Pass, HTML, Audit, Review. All run autonomously once Brain Dump is captured.

Phase 5 is the irreducible human input. Nico's voice signals, on-the-ground reality, family stories, opinions, and ironies cannot be generated by AI. They must be captured.

But Brain Dump is NOT the only source for the article. The Copywriter (Phase 7) integrates four sources:
...
3. **AI knowledge** — generic facts the Copywriter knows (history, architecture, dates)
4. **Search Intent** (Phase 2) — what the reader needs to learn

The Brain Dump enriches and updates. It does not replace the other sources. If a topic is not in the Brain Dump, the Copywriter still covers it using the other three sources, applying Voice DNA throughout.

This understanding shapes Phase 7 Bozza Informativa procedure significantly: see PHASE_07_Bozza_Info.md for the four-source integration pattern.
