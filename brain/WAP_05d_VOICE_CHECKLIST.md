...
| Ellipsis "…" instances | WARN if 0 in section >300 words (informational, not blocking) | Does NOT affect pass/fail status |
...
| Metric | Threshold | Rule |
|---|---|---|
| Sentences per paragraph | max 4 paragraphs in section may exceed 3 sentences, OR ≤50% of paragraphs exceed 3 sentences (whichever is more permissive) | FAIL if more than 4 paragraphs have ≥4 sentences AND >50% of paragraphs exceed 3 sentences |
...
| Metric | Threshold | Rule |
|---|---|---|
| Average sentence length per H2 | PASS if 7-12 words, WARN if 13-15 words, FAIL if >15 words | Lower bound 7 (was 9) to accommodate HTML data-fragment-heavy sections |
...
- FAIL = section/article does not pass, must regenerate
- WARN = informational flag, does NOT block pass status
- Sections can pass with WARNs but cannot pass with FAILs
...
- v1.2 — May 8, 2026 — Calibration round 2 after Test 1 v2 results showed HTML article format differs from video script format. Avg sentence length lower bound 9 → 7 (HTML has more data fragments). Paragraph cap 2 → 4 paragraphs over 3 sentences per section (or 50% threshold). Ellipsis ≥1 demoted from FAIL to WARN (HTML uses other pause devices). Introduced WARN vs FAIL distinction.
