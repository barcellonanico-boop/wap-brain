#!/usr/bin/env bash
# audit_post.sh — WAP article mechanical auditor v0.1
...
echo "============================================"
echo "WAP Article Auditor v0.1 — Category A"
...
# A4 — 0 banned words
BANNED=$(grep -ciE "( SEO |AI Overview|ranking|keywords|schema markup)" "$FILE" || true)
if [ "$BANNED" -eq 0 ]; then
  print_pass "A4 0 banned words"
else
  print_fail "A4 0 banned words" "$BANNED matches found"
  grep -niE "( SEO |AI Overview|ranking|keywords|schema markup)" "$FILE" | head -3 | sed 's/^/       /'
fi

# A5 — 0 Italian-name violations
ITALIAN_V=$(grep -ciE "(Castellamare[^a]|Albergaria|Villa Igea[^,]|Piazza Matrice|Familia[^r])" "$FILE" || true)
if [ "$ITALIAN_V" -eq 0 ]; then
  ...
  grep -niE "(Castellamare[^a]|Albergaria|Villa Igea[^,]|Piazza Matrice|Familia[^r])" "$FILE" | head -3 | sed 's/^/       /'
fi

# A6 — 0 placeholders left
PLACEHOLDERS=$(grep -cE "\[NICO:|\[VERIFY:|TODO|TBD" "$FILE" || true)
...
# A8 — Meta description 140-160 chars
META=$(python3 -c "
import re
with open('$FILE') as f: c = f.read()
...
IMG_URLS=$(grep -oE 'src="[^"]+\.(jpg|jpeg|png|gif|webp|svg)[^"]*"' "$FILE" | sed 's/^src="//;s/"$//' | grep "^http" | sort -u)
IMG_FAILED=0
IMG_TOTAL=0
while IFS= read -r url; do
  [ -z "$url" ] && continue
  IMG_TOTAL=$((IMG_TOTAL + 1))
  STATUS=$(curl -sI -o /dev/null -w "%{http_code}" --max-time 8 "$url" 2>/dev/null || echo "000")
  if [ "$STATUS" != "200" ]; then
...
INT_URLS=$(grep -oE 'href="https?://(www\.)?wearepalermo\.com[^"]*"' "$FILE" | sed 's/^href="//;s/"$//' | sort -u | grep -v "^$")
INT_FAILED=0
INT_TOTAL=0
while IFS= read -r url; do
  [ -z "$url" ] && continue
  INT_TOTAL=$((INT_TOTAL + 1))
  STATUS=$(curl -sI -L -o /dev/null -w "%{http_code}" --max-time 8 "$url" 2>/dev/null || echo "000")
  if [ "$STATUS" != "200" ]; then
...
AFF_URLS=$(grep -oE 'href="[^"]*booking\.com[^"]*"' "$FILE" | sed 's/^href="//;s/"$//' | sort -u)
AFF_TOTAL=$(echo "$AFF_URLS" | grep -c "booking\.com" || true)
AFF_FAILED=0
while IFS= read -r url; do
  [ -z "$url" ] && continue
  STATUS=$(curl -sI -L -o /dev/null -w "%{http_code}" --max-time 10 "$url" 2>/dev/null || echo "000")
  # Accept 200, 301, 302, 403 (anti-bot) as "reachable"
...
BOOKING_TOTAL=$(grep -oE 'href="[^"]*booking\.com[^"]*"' "$FILE" | grep -c "booking\.com" || true)
BOOKING_AID=$(grep -oE 'href="[^"]*booking\.com[^"]*"' "$FILE" | grep -c "aid=918822" || true)
