...
- [ ] 1.2 Fix Via Ambra 11 Airbnb listing — both layers in one pass: (a) public listing (title, description, amenities, photos, host bio, neighborhood USP) and (b) Arrival guide (Check-in method, Wifi details, House manual, Directions, House rules). Draft all copy in Nico voice using WAP_11 as source of truth. Nico pastes into the Airbnb listing editor. [Copywriting] [Est: 90 min | Actual: __ ]
- [ ] 1.3 Write Via Ambra 11 pre-arrival messages — 4 short messages, each with ONE job. Shorter than Via Divisi originals. Do NOT duplicate what Arrival guide already shows. No videos. Messages: (1) welcome + subtle guest-count nudge + expectation-setting, 10 min after booking; (2) ID request + arrival time, 5 days before; (3) airport transport options, 2 days before; (4) day-before nudge with LEFT door callout + "I'm here if you need me", 1 day before [Copywriting] [Est: 45 min | Actual: __ ]
- [ ] 1.4 Set up Airbnb native scheduled messages for Via Ambra 11 — paste the 4 messages into Airbnb scheduled messaging with correct triggers, use Airbnb variables (Guest first name, Check-in date, etc.) where appropriate [PM] [Est: 20 min | Actual: __ ]
- [ ] 1.5 Generate guest-facing in-apartment Apartment Guidebook (Markdown draft) — derived from WAP_11, more detailed than the Airbnb House manual field. Structured for print/PDF. Uses Via Divisi Guidebook as structural template. House Rules + Emergency contact sections at top. All operational info, local recs placeholder for WAP_09/WAP_10 picks. [Copywriting] [Est: 60 min | Actual: __ ]
