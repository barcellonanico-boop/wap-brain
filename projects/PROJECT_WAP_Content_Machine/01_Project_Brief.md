...
| Apr 24, 2026 | SOP_03 word count discipline: 250-350 target, 400 hard cap | Based on S001-S004 being right length and S006 flagged too long. Over-cap stories trigger agent pushback (cut, split, or accept with flag), never silent compliance. |
| Apr 24, 2026 | Story Bank tag taxonomy is closed | No invented tags. New tags require Nico approval in-session and get added to taxonomy as part of same SOP run. Resolves the Apr 22 process flag from S006 intake where Copywriting agent invented 10 tags. |
| Apr 24, 2026 | 70%+ thematic overlap triggers dedupe flag in SOP_03 | Near-duplicates become variations appended to existing file, not new S###. Never silent near-duplicate creation. |
| Apr 24, 2026 | SOP_03 is two-stage, one-agent, one-chat | Stage 1 (craft) and Stage 2 (store) handled by same Story Agent in same session. No hand-off between agents. Pairs-with field is bidirectional — both index rows update when new pair created. |
