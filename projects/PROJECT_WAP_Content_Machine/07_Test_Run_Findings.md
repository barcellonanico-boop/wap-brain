...
### May 1 where-to-stay-palermo run — Findings #56-60
...
These seven patches are the same lesson, applied at seven layers (per-step Architect data sourcing, per-agent boundaries, per-claim hygiene, per-prompt scope caps, per-tool Claude Code scope, per-Intake URL verification, per-Step-4 reader flow validation). All trace to a single root cause: agents (and PM) optimize for surface quality without verifying canonical correctness. Apply as one commit post-publish.
