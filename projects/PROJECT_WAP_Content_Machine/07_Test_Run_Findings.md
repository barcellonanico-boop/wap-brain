...
### May 1 where-to-stay-palermo run — Findings #56-59
...
These six patches are the same lesson, applied at six layers (per-step Architect data sourcing, per-agent boundaries, per-claim hygiene, per-prompt scope caps, per-tool Claude Code scope, per-Intake URL verification). All trace to a single root cause: agents act on assumptions instead of verifying canonical state. Apply as one commit post-publish.
