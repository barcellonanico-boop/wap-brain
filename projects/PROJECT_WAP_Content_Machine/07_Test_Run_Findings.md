...
| 70 | May 1, where-to-stay-palermo Step 6 Scout | Copywriter + Pass 1 brief gap | High | Scout caught 4 of 6 hotel affiliate URLs in Pass 1.5 v2 don't match WAP_12 canonical. Pattern: Copywriter inferred Booking URLs from hotel names instead of pulling verbatim from WAP_12. | Park to post-publish patches batch. Pass 1 Copywriter brief must require every hotel affiliate URL pulled VERBATIM from WAP_12. |
| 71 | May 2, where-to-stay Pass 2 v3-v6 cycles | PM | High | PM example-as-template anti-pattern. WAP_05b and WAP_05 contain example phrasings (e.g., "I know, I know" disarming opener as one option among many). PM treated this example as a default template, generating it as the opener for v3-v6 deliverables. Nico flagged: "that was a fucking example. Now you start every article like that." Same root-cause class as #56-#70: surface-pattern matching instead of contextual generation. | Patched May 2 in PM_SYSTEM_PROMPT Hard Rule 1: Examples in instructions are EXAMPLES not TEMPLATES. PM generates fresh examples per article context. Never copies verbatim from instruction examples. |
| 72 | May 1-2, where-to-stay Pass 2 (full cycle) | Copywriter agent | Critical | Copywriter agent cannot generate Nico-voice content from rules + examples. WAP_05b was created Apr 28 to fix Favignana voice failures. Where-to-stay May 1-2 had WAP_05b in place AND failed Pass 2 four consecutive cycles (v3, v4, v5, v6). Each cycle: agent received voice memo, paraphrased to "improve rhythm," output read as memo-shaped prose, Nico rejected. PM_VOICE_HANDOFF_v3 from this morning DIAGNOSED the transcription-vs-rebuild failure correctly, then v5 (written by the same agent that wrote the diagnosis) FAILED ON THE SAME DIAGNOSIS. Reading the rule and executing the rule are different skills. | Patched May 2 in SOP_01 v2.2: Pass 2 redefined as mechanical assembly from brain dump verbatim, not "voice install on Pass 1 prose." Patched in COPYWRITER_SYSTEM_PROMPT v2.1: Assembler Principle locked. The agent assembles. The agent does not generate voice. |
| 73 | May 2, where-to-stay Pass 2 cycles | PM | High | Pass 2 had no hard stop on Copywriter cycles. PM dispatched v3, then v4, then v5, then v6 over ~9 hours. Each cycle ate ~2-3 hours. After 4 failed cycles, PM took over directly and produced v8 in ~30 min from Nico's brain dump. The PM-direct path that ultimately worked could have started after v2 failure, saving ~6 hours. | Patched May 2 in SOP_01 v2.2 + PM_SYSTEM_PROMPT Hard Rule 2: Pass 2 hard stop at 2 Copywriter cycles. After v2 fails, PM either does direct rebuild from brain dump OR Nico writes Pass 2 himself. No v3 dispatch. |
| 74 | May 1-2, where-to-stay full run | SOP_01 v2.1 design assumption | Critical (root cause) | SOP_01 v2.1 was built on the assumption that the Copywriter agent is the writer and Nico is the approver. Reality across two consecutive runs (Favignana, where-to-stay): Nico is the writer (via brain dumps and voice memos), Copywriter is the assembler. Every locked version of every article traces back to a Nico brain dump verbatim, lightly restructured. The procedure was structured to fight this reality rather than work with it. | Patched May 2 in SOP_01 v2.2: Step 5 Brain Dump session added. Voice memos transcribed VERBATIM by PM. Pass 2 ASSEMBLES from brain dump, does not generate voice from rules. The procedure now matches what actually works. |
| 75 | May 2, where-to-stay v7+v8 chat-only artifacts | PM | High | PM produced v7 and v8 of `07_Pass2_Voice.md` directly in chat. Dispatched Pass 3 HTML brief referencing "v8 locked." But v7 and v8 were never committed to the repo. Copywriter (Pass 3 agent) caught the discrepancy: "v8 doesn't exist in the repo. Repo has v6." PM had to scramble to commit v8 retroactively. Pattern: procedural state lived in chat, not in disk. | Patched May 2 in PM_SYSTEM_PROMPT Hard Rule 4: All artifacts that affect downstream work MUST reach disk before the next step. No chat-only deliverables. Repo or it doesn't exist. |
| 76 | May 1, findings #61-69 retroactive gap | Claude Code prompt protocol | Medium | Findings #61-69 were drafted in PM chat across May 1 closeout cycles but never landed in the repo's `07_Test_Run_Findings.md` file. Earlier closeout prompts referenced "Find row #X" anchors that didn't exist because prior prompts had failed/skipped. Claude Code looked for previous numbered row, found different state than expected. 9 findings exist only in chat history. Discovered May 2 when Claude Code flagged the numbering gap committing #70. | Patched: Claude Code prompts now include verification step (does row #X exist before append #X+1?) AND recovery action when prior state doesn't match. Findings #61-69 partial reconstruction parked for separate batch session — not blocking SOP_01 v2.2 work. | Scout caught 4 of 6 hotel affiliate URLs in Pass 1.5 v2 don't match WAP_12 canonical: Politeama Apartments (draft: politeama-apartments vs WAP_12: painpa-i-mercati-apartments), Hotel Wagner (draft: hotelwagner vs WAP_12: grand-wagner), L'Officina di Apollo (draft: officina-di-apollo vs WAP_12: l-39-officina-di-apollo apostrophe URL-encoding), Unico Boutique Hotel d'Arte (draft: unico-boutique vs WAP_12: unico-boutique-d-39-arte same encoding). Pattern: Copywriter inferred Booking URLs from hotel names instead of pulling verbatim from WAP_12 source-of-truth. Real revenue bug — wrong URLs may 404 or route to wrong hotels. Same root cause class as #56-#60: agents acting on inference instead of verifying canonical source. | Park to post-publish patches batch. Pass 1 Copywriter brief must require: every hotel affiliate URL pulled VERBATIM from WAP_12. No URL inference from hotel names. Add to Pass 1 mandatory self-check (D12 patch extension): "Affiliate URL audit: every URL in draft cross-checked against WAP_12 verbatim. List any deviations." Same lesson as #56 (PM trusted Ubersuggest), #58 (Claude Code inferred scope), #59 (PM assumed URL), #60 (PM approved backwards structure). H2 1 was "Why most where-to-stay advice is wrong" — a defensive opening that announces the article's superiority before delivering value. The whole structure optimized for AIO citation (Quick Picks first, decision matrix early) and forgot that humans need context before decisions. Macro → micro reader flow was missing entirely: pricing came before areas, accommodation type was buried late, horror stories were weaponized as a section instead of sprinkled in voice. PM gated Step 4 and called it "the strongest deliverable of the whole project so far" — wrong. Nico caught the structural failure on read. Same class as Findings #56 (PM trusts proxy data), #58 (agents do work outside scope without verification): PM optimized for surface quality (formatting, comprehensiveness, decision count) instead of canonical correctness (does this match how a real reader thinks?). | Park to post-publish patches batch. SOP_01 v2.1 Step 4 (Structural Audit) needs explicit "reader question flow validation" gate. Step 4 deliverable must include a "macro → micro reader flow" section that maps: what would a real reader ask FIRST? What would they ask SECOND? Does the locked outline answer questions in the order a human asks them? If outline doesn't match the natural question flow, outline is wrong. Plus PM gating rule: PM cannot approve a Step 4 deliverable without independently walking through the "real reader" test on the outline. PM approving on surface quality without reader-flow check = same class of failure as approving Ubersuggest "you're #1" on tool output without GSC check. |
...
### May 1-2 where-to-stay-palermo run — Findings #56-76
...
- PM example-as-template anti-pattern: PM_SYSTEM_PROMPT Hard Rule 1 (examples are examples, not templates).
- Pass 2 hard stop at 2 Copywriter cycles: SOP_01 v2.2 Step 6 + PM_SYSTEM_PROMPT Hard Rule 2.
- Brain-dump-first Pass 2 redesign: SOP_01 v2.2 Step 5 (Brain Dump session) + Step 6 (Pass 2 ASSEMBLY) + COPYWRITER_SYSTEM_PROMPT v2.1 (Assembler Principle).
- All procedural artifacts must reach disk: PM_SYSTEM_PROMPT Hard Rule 4.
- WAP_05b Voice Memo Rework Discipline section added.

These twenty-three patches across Findings #56-76 trace to a single root cause: SOP_01 v2.1 assumed the Copywriter agent could generate Nico voice from rules + examples. Two consecutive runs (Favignana Apr 28 voice failures, where-to-stay May 1-2 voice failures) proved this assumption wrong. Patches #56-70 were surface-level (verify before claim, quote verbatim, scope discipline). Patches #71-76 hit the root cause: the procedure was structured to fight reality. SOP_01 v2.2 stops fighting it. Brain dump drives voice. Copywriter assembles. PM hard-stops loops at 2 cycles. All artifacts reach disk before next step. The redesign is committed May 2, 2026, in this same commit.
